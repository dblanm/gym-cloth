...
from gym_cloth.envs import ClothEnv, DiagonalClothEnv
...
def plot_2d(obs, num_steps, tier):
    ...
    plt.savefig("figures/Graph_tier_" + str(tier) + "_t" + str(num_steps))

    plt.close(fig)


def test_time_sequence(cloth_sequence):
    " cloth_sequence shape (T, cloth points, attributes) attributes={x,y,z, u_x, u_y, u_z}"
    cloth_sequence = np.array(cloth_sequence)
    grip_points = cloth_sequence[:, :, -1]
    cloth_points = cloth_sequence[:, :, :3]
    actions = cloth_sequence[:-1, :, 3:6]  # -1 as last action should be zero

    # Compute x_t - x_{t-1
    delta_cloth = np.diff(cloth_points, axis=0)
    delta_cloth_2 = cloth_points

    # Check for the each timepoint that actions correspond to node change
    for t in range(delta_cloth.shape[0]):
        actions_t = actions[t]
        ...
        if max_action != 0. or min_action != 0.:
            ...
            actions_nodes = actions_t[grip_idx, :]
            diff = np.abs(acted_nodes - actions_nodes)
            max_diff = diff.max()
            if max_diff > 1e-7:
                print("Wrong diff between x_t+1-x_t and u_t diff=", max_diff)
...
if __name__ == "__main__":
    ...
    args = pp.parse_args()
    assert args.tier in [1,2,3,4], args.tier
