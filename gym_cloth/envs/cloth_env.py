...
class ClothEnv(gym.Env):
    ...
    def _pull(self, i, iters_pull, x_diag_r, y_diag_r):
        ...
        return x, y, z, gripped

    def step(self, action, initialize=False, iters_rest=None):
        """Execute one action.
        ...
        if iters_rest is not None:
            iters_resting = iters_rest
        else:
            iters_resting = self.iters_rest
        ...
        iterations = self.iters_up + self.iters_up_rest + iters_pull + self.iters_grip_rest + iters_resting
        ...
        logger.debug("itrs up / wait / pull / wait / drop+rest: {}, {}, {}, {}, {}".format(
            self.iters_up, self.iters_up_rest, iters_pull, self.iters_grip_rest, iters_resting))
        ...
        points_before_pull = self.obs_1d
        # Iterate T times to perform pulling in the diretion of X and Y
        while i < iterations:

            # idx_before_pull = self.grasped_pts
            a_x, a_y, a_z, gripped = self._pull(i, iters_pull, x_dir_r, y_dir_r)
            # Get the points with the action before applying it to the points
            idx_pulled = self.grasped_pts
            if idx_pulled.size != 0:
                initial_points[idx_pulled, 3:6] = np.array([a_x, a_y, a_z])
                if gripped == 0:
                    print("gripping = 0")
                initial_points[idx_pulled, -1] = gripped
            cloth_3d_obs.append(initial_points)
            # idx_after_pull = self.grasped_pts
            # points_after_pull = self.obs_1d
            self.cloth.update()
            initial_points = self._get_state_action(0., 0., 0.)
            # idx_after_update = self.grasped_pts
            # points_after_update = self.obs_1d
            # if a_x != .0 or a_y != .0 or a_z != .0:
            #     print("actions a_x=", a_x, " a_y=", a_y, " a_z=", a_z)
            #     points_before_pull = points_before_pull[idx_before_pull]
            #     points_after_pull = points_after_pull[idx_before_pull]
            #     points_after_update = points_after_update[idx_before_pull]
            #     print("Before pull", points_before_pull)
            #     print("After pull", points_after_pull)
            #     print("After update", points_after_update)
            #     print("Action != 0")

            # Create the state-actions array
            ...
            if self.cloth.have_tear:
                ...
                break
            i += 1
        logger.debug("Finished after {} iterations".format(i))
        # Once the iterations finish we append the last state of the cloth (last cloth update)
        cloth_3d_obs.append(self._get_state_action(0., 0., 0.))
    ...
    def _reset_actions(self):
        ...
        if self._init_type == 'tier1':
            ...
            for i in range(1500):
                self.cloth.update()
                # cloth_3d_obs.append(self._get_state_action(a_x=0.0, a_y=0.0, a_z=0.0))
            logger.debug("Cloth settled, now apply actions.")
            # Get the initial state-action of the cloth
            # We don't save this state as the cloth is wrinkled with itself
            # cloth_3d_obs.append(self._get_state_action(0.0, 0.0, 0.0))
            ...
            if self._delta_actions:
                ...
                raise NotImplementedError()
            action0 = self._convert_action_to_clip_space(action0)
            cloth_1d = self.step(action0, initialize=True, iters_rest=630)
            # cloth_3d_obs.extend(cloth_1d)
            ...
            if idx == -25:
                ...
                if self._delta_actions:
                    ...
                    raise NotImplementedError()
                action1 = self._convert_action_to_clip_space(action1)
            # The cloth is still quite wrinkled, we just get the final state
            cloth_1d = self.step(action1, initialize=True, iters_rest=500)
            # cloth_3d_obs.extend(cloth_1d)

            logger.debug("Let's continue simulating to finish the reset().")
            for i in range(500):
                self.cloth.update()
            # Get the starting state of the cloth
            # cloth_3d_obs.append(self._get_state_action(a_x=0.0, a_y=0.0, a_z=0.0))
            ...
            if self._delta_actions:
                ...
                raise NotImplementedError()
            action0 = self._convert_action_to_clip_space(action0)
            cloth_1d = self.step(action0, initialize=True, iters_rest=1000)
            # cloth_3d_obs.extend(cloth_1d)
