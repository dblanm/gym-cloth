...
def set_floor_pose(floor_mesh_name):
    bpy.data.objects[floor_mesh_name].select_set(state=True)
    bpy.context.view_layer.objects.active = bpy.data.objects[floor_mesh_name]
...
def set_bed_pose(bed_mesh_name):
    bpy.data.objects[bed_mesh_name].select_set(state=True)
    bpy.context.view_layer.objects.active = bpy.data.objects[bed_mesh_name]
...
def set_bed_color(bed_mesh_name, color):
    ...
    alpha = 1
    bpy.context.object.active_material.diffuse_color = (c[0], c[1], c[2], alpha)
...
def set_cloth_color(mesh_name, color_noise, specular_max, init_type, init_side, backfacing=False):
    ...
    bpy.data.objects[mesh_name].select_set(state=True)
    ...
    if backfacing:
        ...
        material_node = node_tree.nodes.new("ShaderNodeOutputMaterial")
...
def main():
    ...
    # bpy.data.lights["Light"].shadow_method = 'NOSHADOW'
    bpy.data.lights["Light"].use_shadow = False
    bpy.data.lights["Light"].falloff_type = 'CONSTANT'
    bpy.data.lights["Light"].energy = 1.5
